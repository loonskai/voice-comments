import { commands, ExtensionContext, StatusBarItem, StatusBarAlignment, window } from 'vscode';
...
class StatusbarUi {
	...
	static get statusbar() {
		if (!StatusbarUi._statusBarItem) {
			StatusbarUi._statusBarItem = window.createStatusBarItem(StatusBarAlignment.Left, 100);
			this.statusbar.show();
		}
...
export function activate(context: ExtensionContext) {
	...
	const startRecording = commands.registerCommand('extension.startRecording', () => {
		StatusbarUi.Recording();
		window.showInformationMessage('Start recording');
	});

	const stopRecording = commands.registerCommand('extension.stopRecording', () => {
		StatusbarUi.Stopped();
